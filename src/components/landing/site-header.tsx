...
export function SiteHeader() {
  ...
  return (
    <>
      <a href="#main-content" className="sr-only focus:not-sr-only focus:absolute focus:z-[100] focus:top-2 focus:left-2 focus:rounded-md focus:bg-gold-500 focus:px-4 focus:py-2 focus:text-ink-900 focus:font-semibold focus:shadow-lg">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3.5 md:px-10 md:py-5">
          <Link href="/" className="group flex items-center gap-3">
            <div className="relative">
              <div className="absolute inset-0 -m-1 rounded-full bg-gold-500/20 blur-md transition-opacity duration-500 group-hover:opacity-100 opacity-0" aria-hidden="true" />
              <Image
                src="/logo.jpg"
          ...
          <nav className="hidden items-center gap-9 md:flex" aria-label="Navegação principal">
            {NAV_LINKS.map((link) => {
              ...
              return (
                ...
                >
                  {link.label}
                  <span className="absolute -bottom-1.5 left-0 h-px w-0 bg-gold-500 transition-all duration-500 group-hover:w-full" aria-hidden="true" />
                </Link>
              )
            })}
          </nav>
          ...
          >
            <span className={`h-0.5 w-6 bg-ink-900 transition-all ${open ? "translate-y-2 rotate-45" : ""}`} />
            <span className={`h-0.5 w-6 bg-ink-900 transition-all ${open ? "opacity-0" : ""}`} />
            <span className={`h-0.5 w-6 bg-ink-900 transition-all ${open ? "-translate-y-2 -rotate-45" : ""}`} />
          </button>
        </div>
        ...
        >
          <nav className="flex flex-col gap-1 px-5 py-5" aria-label="Navegação mobile">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mt-3 inline-flex items-center justify-center gap-2 rounded-full bg-gold-500 px-5 py-3 text-sm font-semibold text-ink-900 min-h-[44px]"
              tabIndex={open ? 0 : -1}
            >
              Falar com o Rabino
            </a>
